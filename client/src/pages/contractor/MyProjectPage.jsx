...
function CircleProgress({ pct, size = 88, stroke = 8 }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="url(#cpg)" strokeWidth={stroke}
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #EAECF4", padding: "0 40px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 10px rgba(91,79,207,0.05)", animation: "fadeIn 0.4s ease" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#9095B0" }}>All projects assigned to you</div>
          </div>
        ...
        <div style={{ padding: "36px 40px" }}>
          ...
          <div style={{ animation: "fadeUp 0.5s ease forwards", opacity: 0, marginBottom: 10 }}>
            <div style={{ fontSize: 11, color: "#5B4FCF", fontWeight: 700, letterSpacing: "1.4px", textTransform: "uppercase", marginBottom: 6 }}>Active Builds</div>
            <h1 style={{ fontSize: 26, fontWeight: 900, color: "#1A1B3A", margin: "0 0 8px", letterSpacing: "-0.5px" }}>My Projects</h1>
            <p style={{ fontSize: 13, color: "#6B7280", margin: 0, lineHeight: 1.7 }}>"Build with precision. Every update you post keeps your customer's trust intact."</p>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div key={project._id} className="proj-card" style={{
                  ...
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    ...
                    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ fontSize: 15, fontWeight: 900, color: "#5B4FCF", letterSpacing: "-0.5px", lineHeight: 1 }}>{pct}%</div>
                      <div style={{ fontSize: 9, color: "#9095B0", fontWeight: 600, marginTop: 2 }}>Done</div>
                    </div>
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                      <div>
                        <h2 style={{ fontSize: 17, fontWeight: 900, color: "#1A1B3A", margin: "0 0 4px", letterSpacing: "-0.3px" }}>{project.projectName}</h2>
                    ...
                    <div style={{ background: "#EEF0F7", borderRadius: 8, height: 7, overflow: "hidden", margin: "12px 0 6px" }}>
                      <div style={{ height: 7, borderRadius: 8, background: "linear-gradient(90deg,#5B4FCF,#7B6FE8)", width: `${pct}%`, transition: "width 1.2s ease" }} />
                    </div>

                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
                      <div style={{ fontSize: 12, color: "#B0B5C8" }}>
                        {pct < 30 ? "🏗 Just started" : pct < 70 ? "⚡ In progress" : "🎯 Nearly done"}
                      </div>
                      <Link to={`/contractor/project/${project._id}`} className="view-btn" style={{
            ...
            {projects.length === 0 && (
              <div style={{ background: "#fff", borderRadius: 20, padding: "64px 40px", textAlign: "center", border: "1px dashed #D1D5E8", animation: "fadeUp 0.5s ease 0.15s forwards", opacity: 0 }}>
                ...
                <div style={{ fontSize: 13, color: "#9095B0", lineHeight: 1.6 }}>Projects assigned to you will appear here.</div>
              </div>
            )}
          </div>
        </div>
