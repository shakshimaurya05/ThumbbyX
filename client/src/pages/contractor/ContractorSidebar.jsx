...
const NAV = [
  { label: "Dashboard",    icon: "⊞",  to: "/contractor/dashboard" },
  { label: "My Projects",  icon: "🏗",  to: "/contractor/my-projects" },
  { label: "My Leads",     icon: "🎯", to: "/contractor/my-leads" },
  { label: "Earnings",     icon: "💰", to: "/contractor/earnings" },
...
export default function ContractorSidebar() {
  ...
  return (
    <div className="portal-sidebar" style={{
      ...
      <div style={{ padding: "28px 22px 24px", borderBottom: "1px solid #EAECF4" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#9095B0", marginTop: 1 }}>Contractor Panel</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: "16px 12px", overflowY: "auto" }}>
        ...
        {NAV.map((item) => {
          const isActive = item.to && (
            ...
            (item.to === "/contractor/my-projects" && location.pathname.startsWith("/contractor/project"))
          );
          return item.to ? (
     ...
     {/* Profile footer */}
      <Link to="/contractor/edit-profile" style={{ textDecoration: "none" }}>
        <div style={{ padding: "14px 20px", borderTop: "1px solid #EAECF4", cursor: "pointer" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg,#5B4FCF,#7B6FE8)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 13, fontWeight: 800, flexShrink: 0 }}>C</div>
            <div style={{ overflow: "hidden" }}>
              ...
              <div style={{ fontSize: 11, color: "#9095B0" }}>Edit Profile →</div>
            </div>
          </div>
        </div>
      </Link>
